...
__attribute__((unused)) static int utf8_decode(const char *s, int *len) {
...
static char **pretokenize(const char *text, int *num_tokens) {
	...
	while (*p) {
		...
		if (is_punctuation((unsigned char)*p)) {
			...
			while (*p && !is_whitespace((unsigned char)*p) && !is_punctuation((unsigned char)*p)) {
...
static int *bpe_tokenize_word(iris_tokenizer *tok, const char *word, int *num_tokens) {
	int len = strlen(word);
	if (len == 0) {
		*num_tokens = 0;
		return NULL;
	...
	while (*p) {
		...
		if (id < 0) {
			...
			for (int i = 0; i < char_len; i++) {
				char byte_buf[8];
				snprintf(byte_buf, sizeof(byte_buf), "<0x%02X>", (unsigned char)p[i]);
	...
	while (changed && n > 1) {
		...
		for (int i = 0; i < n - 1; i++) {
			for (int m = 0; m < tok->num_merges; m++) {
				if (tok->merges[m].left == tokens[i] && tok->merges[m].right == tokens[i + 1]) {
		...
		if (best_idx >= 0) {
			for (int m = 0; m < tok->num_merges; m++) {
				if (tok->merges[m].left == tokens[best_idx] && tok->merges[m].right == tokens[best_idx + 1]) {
...
iris_tokenizer *iris_tokenizer_load(const char *path) {
	...
	/* Read config */
	uint32_t config[8];
	if (fread(config, sizeof(uint32_t), 8, f) != 8)
		goto error;
	...
	for (int i = 0; i < tok->vocab_size; i++) {
		...
		if (!tok->vocab[i])
			goto error;
		if (fread(tok->vocab[i], 1, len, f) != len)
			goto error;
		tok->vocab[i][len] = '\0';
	...
	if (tok->num_merges > 0) {
		...
		for (int i = 0; i < tok->num_merges; i++) {
			uint32_t merge[3];
			if (fread(merge, sizeof(uint32_t), 3, f) != 3)
				goto error;
...
int *iris_tokenize(iris_tokenizer *tok, const char *text, int *num_tokens, int max_len) {
	if (!tok || !text) {
		*num_tokens = 0;
		return NULL;
	}
